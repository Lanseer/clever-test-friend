...
const failureScenarios: FailureScenario[] = [
  ...
  },
  {
    ...
    type: "环境不可用",
    reasoning:
      '测试在第 1 步执行失败：访问目标登录页 "https://sctocloud.com/auth/login" 连接超时（30s 内未响应），目标服务器返回 502 Bad Gateway。环境监控显示该 SIT 环境正在进行例行重启与版本部署，HTTP 探针连续 5 次失败，AI 判定为环境不可用，无法继续后续步骤，且该问题不属于脚本/案例本身缺陷，无法通过自愈修复。',
    fixSteps: [
      "进入「环境管理」页面，确认 SIT 环境当前状态及预计恢复时间。",
      "若需要立即执行，可在右侧配置中将「环境」切换为可用的 UAT 或备用环境 URL。",
      "联系环境运维团队确认服务恢复，必要时在案例备注中记录本次环境异常时间窗。",
      "环境恢复后建议先做一次手工冒烟，再批量重新执行相关案例。",
    ],
    retryHint: "本次失败由环境引起，无需修改案例。请等待环境恢复或切换可用环境后，重新发起现场测试。",
  },
  {
    category: "被测系统/环境问题",
    type: "页面元素变更",
